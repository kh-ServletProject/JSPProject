		...
		<div id="mainContent">
			...
			<div id="content">
				<table border=1 id="hidetr">
					<thead>
						<tr>
							<th id="selectAllMemo" style="display: none;"><input
								id="checkAll" type="checkbox">선택</th>
							<th>번호</th>
							<th>제목</th>
							<th>생성 날짜</th>
							<th>수정 날짜</th>
						...
						<form action="/memo/multiBin" method="post" id="multiBinForm">
							<c:forEach var="memo" items="${ requestScope.memoList }"
								...
								<c:if test="${memo.deleted.toString() == 'n'}">
									<tr>
										<td class="chkTd" style="display: none;">
											<input class="chekedStatus" type="checkbox" name="memoNo" value="${memo.memoNo}">
										</td>
										<td>${ memo.memoNo }</td>
										<td class="title">
											<a href="/memo/detail?memoNo=${ memo.memoNo }">${ memo.memoTitle }</a>
										</td>
